...
function AdminDashboard() {
      ...
      const calculateStats = (data) => {
            ...
            for (let item in itemRatings) {
                  const avg =
                        itemRatings[item].reduce((a, b) => a + b, 0) /
                        itemRatings[item].length;
            ...
            const foods = Object.keys(itemRatings).map(food => ({
                  food,
                  rating: parseFloat(
                        (itemRatings[food].reduce((a, b) => a + b, 0) /
                              itemRatings[food].length).toFixed(2)
                  )
                                    ...
                                    {feedbacks.map((f, i) => (
                                          <div key={i}
                                                ...
                                                <div className="text-teal-600 font-semibold">
                                                      {((f.tasteRating + f.cleanlinessRating + f.staffBehaviourRating) / 3).toFixed(1)} ⭐
                                                </div>
